...
function runner($Query,$Col,$Obj){
	$QueryR = mysqli_query($Obj,$Query);
	$Run = mysqli_fetch_assoc($QueryR);
	$Val = $Run[$Col];
	return $Val;
}
function total_Departments_Agencies(){
	global $con_SCPR, $VendorID, $CertificationID, $FirmID, $PartnerID;
	//$Query = 'SELECT DISTINCT(COUNT(`Business Unit Name`)) AS TDA FROM `scprs`';
	$Query = 'SELECT COUNT(DISTINCT(`Business Unit Name`)) AS TDA FROM `scprs`';
	$Val = runner($Query,'TDA',$con_SCPR);
	return number_format($Val,0); 
}
function total_State_Spend(){ 
	global $con_SCPR, $con_AWT, $VendorID, $CertificationID, $FirmID, $PartnerID;
	//$Query = 'SELECT SUM(`Award Total`) AS TotalAward FROM `scprs`';
	//$Query = 'SELECT SUM(TotalAward) AS t FROM( SELECT SUM(`Award Total`) AS TotalAward FROM `scprs` GROUP BY  `Purchase Order ID` ) B';
	$Query = 'SELECT SUM(`Award Total`) AS t
				FROM `scprs` m
				INNER JOIN 
				(
				SELECT s.scprs_record_id
				FROM scprs s
				GROUP BY s.`Purchase Order ID`) AS tbl ON tbl.scprs_record_id=m.scprs_record_id';
	$Val = runner($Query,'t',$con_SCPR);
	
	$Query1 = 'SELECT SUM(`award_amount`) AS PrimeTotalAward FROM `prime_contractor`';
	$Val1 = runner($Query1,'PrimeTotalAward',$con_AWT);
...
function total_Contracts(){
	...
	$Val = runner($Query,'BUM',$con_SCPR);
	
	$Query1 = 'SELECT DISTINCT(COUNT(`contract_number`)) AS PUM FROM `prime_contractor`';
	$Val1 = runner($Query1,'PUM',$con_AWT);
	
	$Total = $Val+$Val1;	
	return $Total;
}
function total_Small_Business(){
	global $con_SCPR, $con_MAIN, $VendorID, $CertificationID, $FirmID, $PartnerID;
	$Query = 'SELECT COUNT(`Certification ID`) AS CI FROM `sbdvbe`';
	$Val = runner($Query,'CI',$con_MAIN);
	
	$Query1 = 'SELECT COUNT(`Firm ID`) AS FI FROM `dbe`';
	$Val1 = runner($Query1,'FI',$con_MAIN);
	
	$Total = $Val+$Val1;	
	return $Total;
}
function TimeFrequency($Frequency){
	$WHERE = '';
	if($Frequency == 'Today'){
