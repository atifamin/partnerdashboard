...
<div class="col-sm-12" style="padding-top:25px;">
	<div class="widget-box" style="float: left; width: 100%;">
		<div class="widget-header widgetheading" style="background-color:#337ab7;color:white;padding:5px;border-style:solid;border-color:#d2d6de;margin-bottom: 5px;">
			<h4 class="smaller"><i class="ace-icon fa fa-eye"></i> THE BASICS: Increasing Contract Win Rate</h4>
		</div>
		<div class="widget-body">
		<?php
			$Tab2_Q2 = 'SELECT `bidwin_id`,`title`,`content_code` FROM `bidwin` WHERE `type`="Basics" GROUP BY `content_code`';
	...
	<!-- <div class="nav-tabs-custom"> -->
	   <div class="row" style="background:white;margin-left:1px;margin-right:1px;">
		...
		<div class="col-lg-8">
		<div class="tab-content" style="min-height:246px;padding:0%;">
			<?php
				$Tab2_Q3 = 'SELECT * FROM `bidwin` WHERE `type`="Basics" GROUP BY `content_code`';
				$Tab2_Q3R = mysqli_query($con_MAIN,$Tab2_Q3) or die(mysqli_error());
				$Tab2_Index2 = 1;
				while($Tab2_Q3D = mysqli_fetch_assoc($Tab2_Q3R)){
			?>
			<div style="background:aliceblue;height:258px;" id="<?php echo replace_string($Tab2_Q3D['content_code'],'_',' '); ?>" class="tab-pane <?php if($Tab2_Index2 == 1){echo 'in active';} ?>">
				<h3 style="color:white;margin-top:0px;background:#1f497d;padding:2%;font-size: 25px;"><?php echo $Tab2_Q3D['title']; ?></h3>
				<p style="padding:4%;"><?php echo substr($Tab2_Q3D['text'],0,500); ?><br>
				<a  <?php if($Tab2_Q3D['ref_url_target'] == 'BLANK'){echo 'target="_blank"';}?> href="<?php echo $Tab2_Q3D['ref_url']; ?>" style="color:#1f497d;font-size:19px;font-weight:700;" class="pull-right moreinfoa">MORE RESOURCES</a>
				</p>
			</div>
			<?php $Tab2_Index2++; } ?> 
		</div>
