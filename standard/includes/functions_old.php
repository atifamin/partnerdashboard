...
function count_messages(){
    ...
    $result=exec_sqlQuery("SELECT * FROM `norcom_messages` WHERE `message_read_status`= 1 AND `message_recipient_user_id` = '".$user_id."' ");
...
function  norcom_messages()
   ...
   echo '<li id="exup"><a class="link" style="cursor: pointer;" type="button"  data-toggle="modal" data-target="#myModal'.$row['message_id'].'" ><div class="pull-left">
 ...
 function count_notifications(){
    $user_id = $_SESSION['user_id'];
    $result = exec_sqlQuery("SELECT * FROM `norcom_notifications` WHERE `notification_read_status` = 0 AND notification_recipient_user_id = '".$user_id."' ");
    //print_r($result);
	return $result->num_rows;
}
    ...
    while($row = mysqli_fetch_assoc($result)){
       ...
       echo '<li id="epu"><a class="link" style="cursor: pointer;" type="button" onclick="project_update_fn(\''.$row['user_fname'].'\','.$row['project_update_id'].',\''.reply_time($row['project_update_timestamp']).'\','.$row['project_update_sender_user_id'].','.$row['project_update_recipient_user_id'].',\''.$row['project_update_title'].'\',\''.$row['project_update_completion_percentage'].'\',\''.$row['project_update_status'].'\',\''.trim($row['project_update_text']).'\',\''.$pic.'\')" data-toggle="modal" data-target="#projectup_modal" >
                ...
                <div class="progress xs">
                        <div class="progress-bar progress-bar-aqua" style="width: 100%;background-color:aqua;" role="progressbar"
...
function count_execdb_project_updates(){
    $user_id=$_SESSION['user_id'];
    $result=exec_sqlQuery("SELECT * FROM `execdb_project_updates` WHERE `project_update_read_status`= 1 ");
	return $result->num_rows;
}
